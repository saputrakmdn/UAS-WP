<template>
    <div v-if="records.length > 0" class="bg-white p-2 rounded">
                    ...
                    <Dropdown classes="w-44 p-2 border right-0">
                        ...
                        <template v-slot:items>
                            <ul class="flex flex-col gap-2">
                                <li><a @click.prevent="hiddenColumns.length = 0" href="#" :class="{'bg-sky-500 text-white':hiddenColumns.length === 0}" class="inline-block p-2 w-full rounded"><i class="fas fa-eye mr-2"></i>Show All</a></li>
            ...
            <table class="table-auto w-full text-center">
                ...
                <tr v-for="(record, index) in datatableRecords" :class="{'bg-gray-50':index % 2 !== 0}" :key="record">
                    ...
                    <td v-if="action && record[conditionAction.object] === conditionAction.condition">
                        <button v-for="item in actionItems" @click.prevent="$emit(item.emitName, record)" :class="item.classes"><i :class="item.icon"></i>{{item.name}}</button>
                    </td>
                </tr>
                </tbody>
            </table>
        ...
        <div class="flex flex-col-reverse flex-col md:flex-row md:justify-between gap-2 items-center">
            <span class="text-gray-500">Total Records : {{records.length}}</span>
            <div class="rounded overflow-hidden">
                <button :disabled="currentPage === 0" @click.prevent="previousPage" class="p-2 bg-gray-500 text-white hover:bg-gray-600 transition-all disabled:cursor-not-allowed"><i class="fas fa-arrow-alt-circle-left mr-2"></i>Next</button>
                <button class="p-2 bg-gray-500 text-white">{{(currentPage + 1)}} / {{pageCount}}</button>
                <button :disabled="(currentPage+1) === pageCount" @click.prevent="nextPage" class="p-2 bg-gray-500 text-white hover:bg-gray-600 transition-all disabled:cursor-not-allowed">Previous<i class="fas fa-arrow-alt-circle-right ml-2"></i></button>
            </div>
        </div>
    </div>
    <div v-else class="flex flex-col gap-4 items-center">
        <button v-if="createButton" @click.prevent="$emit('onCreate')" class="p-2 bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 focus:outline-0 rounded"><i class="fas fa-plus mr-2"></i>New Record</button>
        <Alert>
...
const emit = defineEmits(['onCreate','onEdit','onDelete','onRead', 'onAccept', 'onReject', 'seeDetail', 'returnBook'])
